...
module.exports = cds.service.impl(async function () {
  ...
  this.on("CREATE", "administrators", async (req) => {
    try {
      ...
      // Define a mapping of adminType to type values
      const typeMapping = {
        "Power User": 1,
        "Quality User": 7,
      ...
      // Determine the type based on the adminType
      const type = typeMapping[adminType];

      // If adminType is not found in the mapping, default to 1
      const finalType = type !== undefined ? type : 1;
      ...
      let administratorData = {
        ...
        adminType: adminType,
        type: finalType, // Set the type based on the mapping
      };
  ...
  this.on("READ", "deptName", async (req) => {
    try {
      ...
      if (reqname === "Quality User") {
        query = { name: { $ne: "Telecom" } }; //except telecom
